...
# Davomat holatlarini boshqarish uchun viewset
class StatusViewSet(viewsets.ViewSet):
    # permission_classes = [AdminUser]

    # Barcha davomat holatlarini ro'yxatini olish
    ...
    # Bitta davomat holatini olish
    def retrieve(self, request, pk=None):
        status = get_object_or_404(Status, pk=pk)
        serializer = StatusSerializer(status)
        return Response(serializer.data)

    # Yangi davomat holati yaratish
...
# Davomatni boshqarish uchun viewset
class DavomatViewSet(viewsets.ViewSet):
    # Barcha davomatlarni ro'yxatini olish
    def list(self, request):
    ...
    def update_attendance(self, request, pk=None):
        attendance = get_object_or_404(Davomat, pk=pk)
        serializer = DavomatSerializer(attendance, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
